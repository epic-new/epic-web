...
| Action | `[name].action.test.ts` | Authentication, transport adaptation, and error translation through the real Service/Model path |
| Route | `[name].route.test.ts` | HTTP authentication/signature handling, request/response semantics, and the real Service/Model path |
...
- Do not create `[name].behavior.test.ts`: the former server Behavior is now the
  Service, verified by `[name].service.test.ts`.
