...
import { useEditRow } from "../use-edit-row.hook";
...
describe("useEditRow", () => {
  it("keeps dialog state isolated to the table that opened it", () => {
    const client = createTestClient();
    const { result, rerender } = renderHook(
      ({ tableName }) => useEditRow(admin.id, tableName),
      {
        initialProps: { tableName: "test_record" },
        wrapper: queryWrapper(client),
      },
    );

    act(() => result.current.handleOpenDialog(row));
    expect(result.current).toMatchObject({
      isDialogOpen: true,
      selectedRow: row,
    });

    rerender({ tableName: "session" });
    expect(result.current).toMatchObject({
      isDialogOpen: false,
      selectedRow: null,
    });
    ...
    rerender({ tableName: "test_record" });
  ...
  it("keeps every cached variant authoritative and invalidates the actor admin family", async () => {
    ...
    const { result } = renderHook(() => useEditRow(admin.id, "test_record"), {
      wrapper: queryWrapper(client),
    });

    let request!: ReturnType<typeof result.current.handleEditCell>;
    act(() => {
      request = result.current.handleEditCell(existingRecord.id, "title", "After");
    });
    ...
    let updated!: Awaited<typeof request>;
    await act(async () => {
  ...
  it("rejects a sensitive-table write and leaves the real database and cache unchanged", async () => {
    ...
    client.setQueryData(statsKey, statsBefore);
    const { result } = renderHook(() => useEditRow(admin.id, "user"), {
      wrapper: queryWrapper(client),
    });

    let request!: ReturnType<typeof result.current.handleEditCell>;
    act(() => {
      request = result.current.handleEditCell(admin.id, "name", "Blocked");
    });
    await waitFor(() => expect(result.current.isLoading).toBe(true));
...
function tableWithAdmin(): DatabaseTableData {
  return {
    rows: [{ id: admin.id, email: admin.email, name: admin.name, role: admin.role }],
    columns: [],
    total: 1,
    page: 1,
    totalPages: 1,
