...
describe("unbanUser action scenarios", () => {
  it("returns the real unbanned record", async () => {
    ...
    await expect(unbanUser({ userId: target.id })).resolves.toMatchObject({
      success: true,
      data: { id: target.id, banned: false },
    });
  });
});
